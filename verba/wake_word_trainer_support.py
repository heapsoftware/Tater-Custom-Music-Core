...
logger = logging.getLogger("wake_word_trainer_support")


class ProductSupportPlugin(ToolVerba):
    name = "wake_word_trainer_support"
    verba_name = "Wake Word Trainer Support"
    pretty_name = "Wake Word Trainer Support"
    version = "1.0.0"
    min_tater_version = "59"

    usage = '{"function":"wake_word_trainer_support","arguments":{"query":"Which wake word trainer should I use on Apple Silicon?"}}'
    description = (
        "Use when a Discord user asks how to choose, install, configure, train with, troubleshoot, "
        "or understand code in the Apple Silicon or NVIDIA microWakeWord trainers. Do not use for "
        "openWakeWord or NanoWakeWord."
    )
    verba_dec = description
    when_to_use = (
        "Use only in Discord when someone asks for help choosing, installing, configuring, using, "
        "training, troubleshooting, or understanding the code of the Apple Silicon or NVIDIA microWakeWord trainer."
    )
    how_to_use = "Pass the user's complete wake word trainer question in query."
    platforms = ["discord"]
    tags = ["discord", "support", "documentation", "github", "wake-word", "training"]
    routing_keywords = [
        "wake word trainer help",
        "install microwakeword trainer",
        "apple silicon wake word training",
        "nvidia docker wake word training",
        "wake word training error",
        "wake word trainer code",
    ]
    common_needs = ["the user's complete support question"]
    missing_info_prompts = ["Are you using the Apple Silicon or NVIDIA microWakeWord trainer, and what would you like help with?"]
    example_calls = [
        '{"function":"wake_word_trainer_support","arguments":{"query":"Which wake word trainer should I use on Apple Silicon?"}}',
        '{"function":"wake_word_trainer_support","arguments":{"query":"How do I run the NVIDIA microWakeWord trainer with Docker?"}}',
        '{"function":"wake_word_trainer_support","arguments":{"query":"Where does the microWakeWord trainer save the finished model?"}}',
    ]
    argument_schema = {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "The user's complete wake word trainer support or code question.",
            }
        },
        "required": ["query"],
    ...
    waiting_prompt_template = (
        "Tell {mention} you are checking the current wake word trainer docs and code. "
        "Keep it to one short Discord-friendly sentence and output only that sentence."
    )

    repositories: Tuple[Dict[str, str], ...] = (
        {"slug": "TaterTotterson/microWakeWord-Trainer-AppleSilicon", "label": "microWakeWord Trainer for Apple Silicon", "ref": "main"},
        {"slug": "TaterTotterson/microWakeWord-Trainer-Nvidia-Docker", "label": "microWakeWord Trainer for NVIDIA", "ref": "main"},
    )
    product_label = "Tater microWakeWord trainers"
