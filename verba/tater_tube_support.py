...
logger = logging.getLogger("tater_tube_support")


class ProductSupportPlugin(ToolVerba):
    name = "tater_tube_support"
    verba_name = "Tater Tube Support"
    pretty_name = "Tater Tube Support"
    version = "1.0.0"
    min_tater_version = "59"

    usage = '{"function":"tater_tube_support","arguments":{"query":"How do I install Tater Tube on Steam Deck?"}}'
    description = (
        "Use when a Discord user asks how to install, configure, pair, troubleshoot, use, or develop "
        "Tater Tube for Raspberry Pi, Tater Tube for Steam or Steam Deck, or Tater Tube Server, "
        "including questions that require checking their README files or source code."
    )
    verba_dec = description
    when_to_use = (
        "Use only in Discord when someone asks for help installing, configuring, using, "
        "troubleshooting, or understanding the code of Tater Tube, Tater Tube Steam, or Tater Tube Server."
    )
    how_to_use = "Pass the user's complete Tater Tube question in query."
    platforms = ["discord"]
    tags = ["discord", "support", "documentation", "github", "tater-tube", "steam-deck"]
    routing_keywords = [
        "tater tube help",
        "install tater tube",
        "tater tube steam deck",
        "tater tube server setup",
        "tater tube error",
        "tater tube code",
    ]
    common_needs = ["the user's complete support question"]
    missing_info_prompts = ["Are you using the Tater Tube Pi player, Steam/Steam Deck edition, or Tater Tube Server?"]
    example_calls = [
        '{"function":"tater_tube_support","arguments":{"query":"How do I install Tater Tube on Steam Deck?"}}',
        '{"function":"tater_tube_support","arguments":{"query":"How do I pair a player with Tater Tube Server?"}}',
        '{"function":"tater_tube_support","arguments":{"query":"Where is Local TV channel scheduling implemented?"}}',
    ]
    argument_schema = {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "The user's complete Tater Tube support or code question.",
            }
        },
        "required": ["query"],
    ...
    waiting_prompt_template = (
        "Tell {mention} you are checking the current Tater Tube docs and code. "
        "Keep it to one short Discord-friendly sentence and output only that sentence."
    )

    repositories: Tuple[Dict[str, str], ...] = (
        {"slug": "TaterTotterson/Tater-Tube", "label": "Tater Tube for Raspberry Pi", "ref": "main"},
        {"slug": "TaterTotterson/Tater-Tube-Steam", "label": "Tater Tube for Steam and Steam Deck", "ref": "main"},
        {"slug": "TaterTotterson/tater-tube-server", "label": "Tater Tube Server", "ref": "main"},
    )
    product_label = "Tater Tube"
